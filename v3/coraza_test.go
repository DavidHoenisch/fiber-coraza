...
func TestHandleIntervention(t *testing.T) {
	...
	app.Get("/redirect-default", func(c fiber.Ctx) error {
		return handleIntervention(c, &types.Interruption{Action: "redirect", Data: "/login"})
	})
	app.Get("/redirect-status-one", func(c fiber.Ctx) error {
		return handleIntervention(c, &types.Interruption{Action: "redirect", Status: 1, Data: "/login"})
	})
