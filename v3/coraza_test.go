...
func TestHandleIntervention(t *testing.T) {
	...
	app.Get("/redirect-default", func(c fiber.Ctx) error {
		return handleIntervention(c, &types.Interruption{Action: "redirect", Data: "/login"}, Config{})
	})
	app.Get("/redirect-status-one", func(c fiber.Ctx) error {
		return handleIntervention(c, &types.Interruption{Action: "redirect", Status: 1, Data: "/login"}, Config{})
	})
	...
	resp, err = app.Test(httptest.NewRequest("GET", "/redirect-default", nil))
	if err != nil {
		t.Fatalf("default redirect request failed: %v", err)
	}
	if resp.StatusCode != fiber.StatusSeeOther {
		t.Fatalf("default redirect status = %d", resp.StatusCode)
	}

	if _, err = app.Test(httptest.NewRequest("GET", "/redirect-status-one", nil)); err == nil {
		t.Fatal("expected status-one redirect to produce a malformed test response")
	}

	if _, err = app.Test(httptest.NewRequest("GET", "/drop", nil)); err == nil {
		t.Fatal("expected drop to close the test connection")
	}
